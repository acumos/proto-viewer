...
def test_msg_to_json_preserve_bytes(monkeypatch, monkeyed_requests_get, cleanuptmp,
    ...
    jpb = data._msg_to_json_preserve_bytes(msgb, test_proto_mid, "Data1", 1)
    assert (jpb == fake_msg_as_jsonwb())

    msgba = fake_msg_with_arrays()
    jpb2 = data._msg_to_json_preserve_bytes(msgba, test_proto_with_arrays_mid, "ImageTagSet", 1)
    assert(jpb2 == fake_msg_with_arrays_jsonwb())

    cleanuptmp()


def _verify_inject_test(fake_msg_as_jsonwb, fake_msg_with_arrays_jsonwb, test_proto_mid, test_proto_with_arrays_mid):
    expected_h_1 = "{0}{1}{2}".format(test_proto_mid, "Data1", 'asdf0')
    expected_index_1 = hashlib.sha224(expected_h_1.encode('utf-8')).hexdigest().encode()
    expected_h_2 = "{0}{1}{2}".format(test_proto_with_arrays_mid, "ImageTagSet", 'asdf0')
    ...
    assert(data.get_raw_data(test_proto_mid, "Data1", 0, 1) == [fake_msg_as_jsonwb()])
    assert(data.get_raw_data(test_proto_with_arrays_mid, "ImageTagSet", 0, 1) == [fake_msg_with_arrays_jsonwb()])
...
def test_inject_data_env(monkeypatch, monkeyed_requests_get, cleanuptmp,
    ...
    msgb = fake_msg()
    data.inject_data(msgb, test_proto_url, "Data1")
    msgbwa = fake_msg_with_arrays()
    data.inject_data(msgbwa, test_proto_with_arrays_url, "ImageTagSet")
