...
def graphs_change():
    ...
    if graph_val in ["table"]:
        # user can pick the columns of the same size (table height)
        ...
        columnmultiselect = MultiSelect(title="Columns (min 2):", value=[], options=col_options, name=COLUMN_MULTISELECT)
...
def column_selection_change():
    """Callback for changing the selected table column(s)"""
    d = curdoc()
    _remove_fig(d)
    model_id, message_name, _ = run_handlers.get_modelid_messagename_type(d)
    fake_table = False
    if (fake_table):
        data = dict( c1=[random.randint(0, 100) for i in range(10)], c2=[random.randint(0, 100) for i in range(10)])
        source = ColumnDataSource(data)
        columns = [ TableColumn(field="c1", title="Col 1"), TableColumn(field="c2", title="Col 2"), ]
    else:
        sind = run_handlers.get_source_index(d.session_context.id, model_id, message_name)
        source = d.get_model_by_name(sind)
        _install_callback_and_cds(sind, model_id, message_name, stream_limit=100000)
