...
def _compile_proto(model_id):
    ...
    if not isfile(gen_module):
        _logger.error("Expected file {0} did not get created! {1} listing: {12}".format(gen_module, OUTPUT_DIR, listdir(OUTPUT_DIR)))
        raise Exception(
...
def _register_proto(proto_name, model_id):
    ...
    _logger.info("_register_proto: processing model_id {0}".format(model_id))
...
def _wget_proto(url, model_id):
    ...
    if r.status_code == 200:
        ...
        with open(wpath, "w") as f:
            f.write(r.text)
        _logger.debug("_wget_proto: model_id {0}, url {1} downloaded to {2}".format(
            model_id, url, wpath))
